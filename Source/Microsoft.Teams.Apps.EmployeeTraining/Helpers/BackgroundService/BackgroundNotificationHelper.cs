...
namespace Microsoft.Teams.Apps.EmployeeTraining.Helpers.BackgroundService
{
    ...
    public class BackgroundNotificationHelper : BackgroundService
    {
        ...
        /// <summary>
        /// Provides the methods for managing event category operations
        /// </summary>
        ...
        protected async override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (dailyReminderEvents != null && dailyReminderEvents.Any())
                    {
                    ...
                    if (DateTimeOffset.UtcNow.DayOfWeek == DayOfWeek.Monday)
                    {
                        ...
                        if (weeklyReminderEvents != null && weeklyReminderEvents.Any())
                        {
...
#pragma warning restore CA1031 // Catching general exceptions that might arise during execution to avoid blocking next run.
        ...
        private async Task SendReminder(IEnumerable<EventEntity> eventDetails, NotificationType notificationType)
        {
            ...
            await this.BindCategoryDetails(eventDetails);
            ...
            if (userList.Any())
            {
                ...
                foreach (var user in registeredAttendees)
                {
                    ...
                    {
                        var card = ReminderCard.GetCard(filteredEvents.OrderBy(eventDetails => eventDetails.StartDate).Take(10), this.localizer, this.botOptions.Value.ManifestId, notificationType);
                        var userToNotify = new List<User> { user };
                        this.notificationHelper.BroadcastNotificationToUsers(userToNotify, card);
                    }
                }
            }
        }
