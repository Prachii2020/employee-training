...
namespace Microsoft.Teams.Apps.EmployeeTraining.Tests.Helpers
{
    ...
    public class EventWorkflowHelperTest
    {
        ...
        {
            var logger = new Mock<ILogger<EventWorkflowHelper>>().Object;
            ...
            categoryStorageProviderFake = new CategoryStorageProviderFake();
        ...
        {
            var eventDetails = EventWorkflowHelperData.eventEntity;

            this.eventStorageProvider
                .Setup(x => x.GetEventDetailsAsync(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(this.eventStorageProviderFake.GetEventDetailsAsync(eventDetails.EventId, eventDetails.TeamId));

            this.userStorageConfigurationProvider
               .Setup(x => x.GetUserConfigurationsAsync(eventDetails.GetAttendees()))
               .Returns(this.userConfigurationStorageProviderFake.GetUserConfigurationsAsync(eventDetails.GetAttendees()) );

            this.notificationHelper
               .Setup(x => x.BroadcastNotificationToUsers(It.IsAny<IEnumerable<Models.User>>(), It.IsAny<BotSchema.Attachment>()))
               .Returns(new List<string>());

            var result = await this.eventWorkflowHelper.SendReminderAsync(eventDetails.TeamId, eventDetails.EventId);
